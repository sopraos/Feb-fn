...
module.exports = (env) => {
	...
	const jsSrc = path.join(process.env.PWD, PATH_CONFIG.src, PATH_CONFIG.javascripts.src);
	const publicPath = path.join(TASK_CONFIG.javascripts.publicPath || PATH_CONFIG.javascripts.dest, '/');

	// Configuration
	let webpackConfig = {
		// Points d'entrée & Context
		context: jsSrc,
